  ...
  if (isLoading) {
    return (
      <div className="p-6 rounded-2xl bg-gradient-to-br from-grateful-primary/10 to-grateful-secondary/10 border border-grateful-primary/20 animate-pulse mb-8">
        <div className="h-4 bg-gray-300bg-gray-600 rounded w-1/2 mb-4"></div>
        <div className="space-y-2">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="h-12 bg-gray-300bg-gray-600 rounded"></div>
          ))}
        </div>
      </div>
    );
  ...
  if (usersWithWallets.length === 0) {
    return (
      <motion.div
        ...
        className="p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl bg-gradient-to-br from-grateful-primary/10 to-grateful-accent/10from-grateful-primary/15to-grateful-accent/15 border border-grateful-primary/20border-grateful-accent/30 backdrop-blur-sm mb-6 sm:mb-8 shadow-lg bg-white/80bg-slate-900/90"
      >
        ...
        <p className="text-xs sm:text-sm text-slate-600text-slate-400">
  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 max-h-64 sm:max-h-96 overflow-y-auto">
        {usersWithWallets.map((user: any, index: number) => (
          <motion.div
            ...
            className="p-2.5 sm:p-3 rounded-lg sm:rounded-xl bg-white/90bg-slate-800/90 border border-blue-200/20border-blue-700/30 flex items-center justify-between gap-2 sm:gap-3 hover:bg-whitehover:bg-slate-700 transition-colors shadow-md hover:shadow-lg backdrop-blur-sm"
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
                <span className="text-xs font-medium text-slate-600text-slate-400">
                  @{user.twitterHandle || user.username || 'anonymous'}
                </span>
              </div>
              <p className="font-mono text-xs sm:text-sm text-slate-800text-slate-200 break-all drop-shadow-sm">
                {user.walletAddress}
              </p>
            </div>
            <motion.button
              onClick={() => handleCopy(user.walletAddress, index)}
              className="p-1.5 sm:p-2 rounded-lg bg-white/95bg-slate-700/90 hover:bg-whitehover:bg-slate-600 transition-colors flex-shrink-0 shadow-sm hover:shadow-md touch-manipulation"
            ...
            >
              {copiedIndex === index ? (
                ...
                <Copy className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-slate-600text-slate-400" />
              )}
            </motion.button>
          </motion.div>
        ))}
      ...
      <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-grateful-primary/20">
        <p className="text-[10px] sm:text-xs text-slate-600text-slate-400 text-center px-2">
